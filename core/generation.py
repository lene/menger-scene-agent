...
_RULES = (
    ...
    "built-in equivalent.\n"
    "- Animation: an animated scene defines `def scene(t: Float): Scene` instead of `val "
    "scene`, where `t` is time in SECONDS. It must ALSO declare `val duration = <seconds>f` in "
    "the same object (e.g. `val duration = 10f` for \"about ten seconds\"): the render window "
    "then plays t from 0 to duration in real time, looping. Without `duration` the window "
    "shows only t = 0, a still image, so never omit it. The corpus examples predate this and "
    "use other t ranges -- do not copy their ranges. Drive each animated property from "
    "`t / duration` (0 at the start, 1 at the end), e.g. `level = 2f * t / duration` or an "
    "angle of `2 * Pi * t / duration` for one full turn. Everything the request does not ask "
    "to animate stays constant. The validator evaluates the scene at t = 0 AND t = duration, "
    "so both ends must be valid (e.g. a sponge's `level` must stay within its limits at the "
    "end).\n"
    ...
    "- A complaint or observation about the rendered result that implies what it should be "
    "is a CHANGE REQUEST, not a question: \"that's not glass, it looks like matte plastic\" "
    "means make it look like real glass; \"shouldn't there be shadows on the floor?\" means "
    "make shadows appear on the floor; \"the top face has no green\" means fix the colouring. "
    "Work out why the scene does not show the expected result (read the conventions in the "
    "manifest) and fix that cause. Plain commands (\"keep them like that\", \"leave the "
    "coloring as it is\") are change requests too -- if nothing needs to change, return the "
    "scene unchanged.\n"
    "- Only a pure information question with no implied change (\"why does it look like "
    "chrome?\", \"what does proceduralScale do?\") gets NEEDS_CLARIFICATION: this reads as "
    "a question about the current scene, not a change request -- use /ask or /question for an "
    "explanation, or say what to change. \"make it darker\", \"make it slower\" are "
    "directives: pick the single most direct, conventional lever and compose the scene; do "
    "not ask which one was meant. Reserve NEEDS_CLARIFICATION for a genuinely undefined "
    "term, a direct contradiction, a pure question, or an effect the DSL cannot produce.\n"
    "- If the request needs an effect the DSL cannot produce (a glow or halo around an "
    "object -- emission only makes the surface itself flat and bright; light or colour that "
    "falls off with distance from a point; an object type or field not in the manifest), do "
    "not silently approximate it. Respond with NEEDS_CLARIFICATION naming what is not "
    "possible AND the nearest thing that is (e.g. \"no glow or halo exists; the nearest is an "
    "emissive surface, or nested translucent spheres with decreasing emission\").\n"
)
